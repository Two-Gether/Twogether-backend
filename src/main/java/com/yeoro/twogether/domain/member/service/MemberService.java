...
public interface MemberService {
     ...
     */
    Long signupByOauth(String email, String nickname, String profileImage, LoginPlatform loginPlatform, String platformId, String encodedPassword);
     ...
     * 없으면 신규 가입 후 ID 반환
     */
    Long findOrCreateMember(String email, String nickname, String profileImage, LoginPlatform loginPlatform, String platformId, String encodedPassword);

    /**
     * 카카오 로그인 처리
     * OAuth 프로필 조회 → 회원 조회/가입 → JWT 발급 및 클라이언트 전달 → 응답 DTO 반환
     */
    LoginResponse kakaoLogin(String accessToken, HttpServletRequest request, HttpServletResponse response);

    /**
     * 현재 로그인한 사용자 정보 조회
     * JWT 토큰에서 사용자 식별 후 Member 엔티티 반환
     */
     ...
     * 프로필 이미지 수정
     */
    void updateProfileImage(HttpServletRequest request, String newImageUrl);

    /**
     * 닉네임 수정
     */
    void updateNickname(HttpServletRequest request, String newNickname);

    /**
     * 파트너 연결 끊기
     */
    void disconnectPartner(HttpServletRequest request);

    /**
     * 비밀번호 변경 (LOCAL 사용자 전용)
     */
    void updatePassword(HttpServletRequest request, String currentPassword, String newPassword);
}
